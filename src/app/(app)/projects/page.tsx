...
function normalizeStatus(status?: string): ProjectStatus | "semua" {
  if (status === "aktif" || status === "tertunda" || status === "selesai" || status === "arsip") {
    return status;
  }
    ...
    <div className="space-y-6">
      ...
      <SurfaceCard className="space-y-4">
        <div className="flex gap-2 overflow-x-auto pb-1">
          ...
          <TabLink href="/projects?status=aktif" isActive={selectedStatus === "aktif"} label="Aktif" count={counts.aktif} />
          <TabLink href="/projects?status=tertunda" isActive={selectedStatus === "tertunda"} label="Tertunda" count={counts.tertunda} />
          <TabLink href="/projects?status=selesai" isActive={selectedStatus === "selesai"} label="Selesai" count={counts.selesai} />
          <TabLink href="/projects?status=arsip" isActive={selectedStatus === "arsip"} label="Arsip" count={counts.arsip} />
        </div>
